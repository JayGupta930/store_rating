...
import { FaStar, FaSearch, FaSignOutAlt, FaStore, FaMapMarkerAlt, FaEye, FaArrowLeft, FaArrowRight } from 'react-icons/fa';
...
const UserDashboard = () => {
    ...
    const autoPlayDelay = 3000; // 3 seconds between slides for smoother automatic scrolling
        ...
        <div className="user-dashboard">
                ...
                >
                    <div ref={carouselRef} className="modern-carousel">
                        {carouselImages.map((image, index) => (
                            <div 
                                key={index} 
                                className={`modern-carousel-item ${currentSlide === index ? 'active' : ''}`}
                            >
                    ...
                    <div className={`auto-scroll-indicator ${isAutoPlaying ? 'active' : ''}`}>
            ...
            <div className="search-section">
                ...
                <div className="search-categories">
                    <button
                        className={`category-btn ${searchCategory === 'name' ? 'active' : ''}`}
                        onClick={() => handleSearchCategoryChange('name')}
                    >
                        Search by Name
                    </button>
                    <button
                        className={`category-btn ${searchCategory === 'address' ? 'active' : ''}`}
                ...
                {filteredStores.map(store => (
                    <div key={store.id} className="store-card">
                        ...
                        <div className="store-content">
                            ...
                            <p className="store-description">{store.description || 'No description available'}</p>
                                ...
                                <div className="overall-rating">
                                    ...
                                    <div className="stars">
                                        {[...Array(5)].map((_, index) => (
                                            <FaStar
                                                key={index}
                                                className={`star ${index < Math.floor(store.overallRating) ? 'filled' : ''}`}
                                            />
                                        ))}
                                        <span className="rating-value">({store.overallRating?.toFixed(1) || 'N/A'})</span>
                                    </div>
                                </div>
                                <button 
